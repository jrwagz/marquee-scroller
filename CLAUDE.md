...
- Default font: 5px wide + 1px spacer = 6px per character (`width` variable, line 97)
...
- The scroller font is selectable per-device via `display_font` (issue #106). Fifteen fonts
  are registered in `SCROLLER_FONTS[]` near the top of `marquee.ino`: five short fonts
  (Classic, Block, Org, Picopixel, TomThumb), five hand-designed 8-pixel fonts (Tall, Bold,
  Slim, Outline, Digi), and five 8-pixel stylistic variants derived from Tall via procedural
  transforms (Italic, Serif, Pixel, Inverse, Stencil). All custom glyphs live in
  `marquee/WagfamFont.h`, generated by `scripts/gen_wagfam_font.py` from human-readable
  ASCII-art GLYPHS dicts plus the per-font `derive_from=(src_name, transform_fn)` mechanism.
  Org/Picopixel/TomThumb are vendored Adafruit_GFX fonts. The clock face (`centerPrint`)
  always uses the Classic font; only the marquee scroll changes. Custom-font rendering goes
  through `scrollMessageWaitCustomFont()`, which uses Adafruit_GFX's variable-width path
  (`setFont` + `setCursor` + `print`) since the indexed fixed-pitch loop in the Classic path
  doesn't generalize. Tall/Slim/Digi/Italic/Serif/Pixel/Stencil have distinct lowercase
  glyphs (the derived ones inherit Tall's lowercase via the transform); Block/Bold/Outline
  render lowercase a-z using the uppercase bitmap. Inverse renders Tall inverted in the top
  h-1 rows — it's a stylized aesthetic, not a strict-readability font.
