...
- The scroller font is selectable per-device via `display_font` (issue #106). Five fonts are
  registered in `SCROLLER_FONTS[]` near the top of `marquee.ino`: Classic (Adafruit_GFX builtin
  5×7, the default), Block (custom `WagfamBlock` from `marquee/WagfamFont.h` — generated by
  `scripts/gen_wagfam_font.py`), Org, Picopixel, and TomThumb (the latter three are vendored
  Adafruit_GFX fonts). The clock face (`centerPrint`) always uses the Classic font; only the
  marquee scroll changes. Custom-font rendering goes through `scrollMessageWaitCustomFont()`,
  which uses Adafruit_GFX's variable-width path (`setFont` + `setCursor` + `print`) since
  the indexed fixed-pitch loop in the Classic path doesn't generalize
