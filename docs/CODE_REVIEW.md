...
### 6. `displayTime` Rebuilt Every Frame
...
### 7. `firstTimeSync` / `lastRefreshDataTimestamp` — `uint32_t` vs `time_t`
...
### 8. `centerPrint()` Rewrites Display Every Frame
...
### 9. `matrix.shutdown(false)` Called Every Minute Unnecessarily
